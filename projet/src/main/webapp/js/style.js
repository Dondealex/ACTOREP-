...
function choix01(val){
	
	//alert(act.categorie.getId())
	$.ajax({
        //L'URL de la requête 
        url: "http://localhost:8080/services_acteur/"+val,
    ...
    .done(function(response){
	    ...
	    for (i = 0; i < response.length; i++) {
	    	...
	    	$("#services").append('<input type="checkbox" id="service" name="service" value="'+response[i].id+'">')
		      .append('<label for="service">'+response[i].nom+'</label>')
		      .append(`<br>`);
